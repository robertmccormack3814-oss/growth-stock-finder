...
def max_drawdown(prices):
    if len(prices) < 30:
        return None
    peak = prices.cummax()
    return float((prices / peak - 1).min())
...
def analyse(stock, benchmark_close):
    ...
    if exchange and "NASDAQ" not in exchange and exchange not in ("NMS", "NGM", "NCM"):
        raise ValueError(f"not Nasdaq exchange: {exchange}")
    ...
    if price < CFG.get("min_price", 1.0):
        raise ValueError("below minimum price")
    if market_cap is not None and market_cap < CFG.get("min_market_cap", 100_000_000):
        raise ValueError("below minimum market cap")
    if avg_dollar_vol < CFG.get("min_avg_dollar_volume", 500_000):
    ...
    tax_rate = clamp(tax / pretax, 0, 0.35) if tax is not None and pretax and pretax > 0 else 0.21
    ...
    ])) * WEIGHTS["balance_sheet"]

    momentum_score = float(np.mean([
    ...
    pe_to_growth = pe / max(growth_ref * 100, 1) if pe else None
    valuation_score = float(np.mean([
    ...
    size_risk = (300_000_000 / market_cap) if market_cap and market_cap > 0 else None
    risk_score = round(float(np.mean([
        ...
        score_linear(size_risk, 0.0, 1.0),
    ])) * 100, 1)

    risk_grade = "Low" if risk_score < 30 else "Moderate" if risk_score < 50 else "High" if risk_score < 70 else "Very High"

    growth_gate = ((rev_growth is not None and rev_growth >= 0.10) or
                   (earnings_growth is not None and earnings_growth >= 0.15) or
                   (revenue_cagr is not None and revenue_cagr >= 0.10))
    ...
    return {
        "symbol": stock["symbol"],
        "ticker": ticker,
        "company": info.get("longName") or stock.get("company"),
        "market": "NASDAQ",
        "currency": "USD",
        "price": round(price, 3),
        ...
        "component_scores": {
            "growth": round(growth_score, 1), "quality": round(quality_score, 1),
            "balance_sheet": round(balance_score, 1), "momentum": round(momentum_score, 1),
            "valuation": round(valuation_score, 1)
        },
        "metrics": {
            "revenue_growth": pct(rev_growth), "earnings_growth": pct(earnings_growth),
            "revenue_cagr": pct(revenue_cagr), "eps_cagr": pct(eps_cagr),
            "roe": pct(roe), "roic": pct(roic), "gross_margin": pct(gross_margin),
            "operating_margin": pct(op_margin), "fcf_margin": pct(fcf_margin),
            "current_ratio": current_ratio, "debt_to_equity": debt_to_equity,
            "net_debt_ebitda": net_debt_ebitda, "interest_cover": interest_cover,
            "return_6m": pct(ret6), "return_12m": pct(ret12),
            "relative_strength_12m": pct(rs12), "distance_to_52w_high": pct(near_high - 1),
            "forward_pe": forward_pe, "peg": peg, "fcf_yield": pct(fcf_yield),
            "volatility": pct(vol), "max_drawdown": pct(mdd), "beta": beta
        },
...
def main():
    ...
    state = load_json(DATA / "state.json", {"signals": {}, "market": "NASDAQ"})
    cursor = load_json(DATA / "cursor.json", {"next_index": 0})
    start = int(cursor.get("next_index", 0)) % len(universe)
    size = int(os.getenv("BATCH_SIZE") or CFG.get("batch_size", 120))
    batch = [universe[(start + i) % len(universe)] for i in range(min(size, len(universe)))]

    bench = yf.download(CFG.get("benchmark", "^IXIC"), period="2y", auto_adjust=True, progress=False)
    if bench.empty:
        raise SystemExit("Could not download Nasdaq benchmark")
    bclose = bench["Close"]
    ...
    for stock in batch:
        ...
        try:
            ...
            state.setdefault("signals", {})[symbol] = {
                ...
                "risk_score": result["risk_score"], "updated_at": result["updated_at"]
            }
            print(symbol, result["status"], result["score"], "risk", result["risk_score"])
        except Exception as exc:
            old_results[symbol] = {
                "symbol": symbol, "company": stock.get("company"), "market": "NASDAQ",
                "status": "ERROR", "error": str(exc), "updated_at": now_iso()
            }
            print(symbol, "ERROR", exc)

    ranked = sorted(old_results.values(), key=lambda r: (
        r.get("status") == "PASS", r.get("status") == "WATCH", r.get("score", -1)
    ), reverse=True)
    save_json(DATA / "results.json", {r["symbol"]: r for r in ranked})
    save_json(DATA / "ranked.json", ranked)
    state["market"] = "NASDAQ"
    save_json(DATA / "state.json", state)
    save_json(DATA / "cursor.json", {
        "next_index": (start + len(batch)) % len(universe), "last_run": now_iso(),
        "processed": len(batch), "universe_size": len(universe), "market": "NASDAQ"
    })
    print(f"Processed {len(batch)} Nasdaq stocks; next index {(start + len(batch)) % len(universe)}")
